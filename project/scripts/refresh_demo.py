...
def _make_cell_chart():
    ...
    if not hier_path.exists():
        print('  [skip] No hierarchy CSV found.'); return
    ...
    d2 = d2[d2['count'] > 0]
    if d2.empty:
        print('  [skip] No depth-2 regions found.'); return

    colors = ['#E57373','#FF9800','#FFEB3B','#66BB6A','#26C6DA',
              '#5C6BC0','#AB47BC','#EC407A','#26A69A','#8D6E63','#78909C','#FFA726']
    ...
    bars = ax1.barh(range(len(d2)), d2['count'].values,
                    color=[colors[i % len(colors)] for i in range(len(d2))],
                    edgecolor='none', height=0.7)
    ax1.set_yticks(range(len(d2)))
    ax1.set_yticklabels([f"{r['acronym']} ({r['region_name'][:20]})" for _, r in d2.iterrows()],
    ...
    for bar, row in zip(bars, d2.itertuples()):
        ax1.text(bar.get_width() * 1.01, bar.get_y() + bar.get_height() / 2,
                 f'{int(row.count):,}', va='center', ha='left', fontsize=8, color='#aaa')
    total = d2['count'].sum()
    pcts  = d2['count'].values / total * 100
    labels = [f"{r['acronym']} {p:.1f}%" if p > 3 else ''
              for (_, r), p in zip(d2.iterrows(), pcts)]
    ax2.pie(d2['count'].values, colors=[colors[i%len(colors)] for i in range(len(d2))],
            labels=labels, labeldistance=1.1,
            textprops={'fontsize': 8, 'color': '#cccccc'}, startangle=90,
    ...
    plt.savefig(str(chart_path), dpi=120, bbox_inches='tight', facecolor='#0e0e16', edgecolor='none')
    plt.close()
    print(f'  cell_count_chart.png  (total {int(root):,} cells, {len(hier)} regions)')


def _make_best_slice(idx: int):
    from make_demo_panel import _vibrant_recolor, _crop_to_brain
    from PIL import Image as Im, ImageDraw as ID, ImageFont as IF
    import numpy as np, tifffile, glob as _g
    ...
    if not ov_path.exists():
        print(f'  [skip] slice_{idx:04d}_overlay.png not found.'); return

    ov  = np.array(Im.open(str(ov_path)).convert('RGB'))
    lbl = tifffile.imread(str(lbl_path)) if lbl_path.exists() else np.zeros(ov.shape[:2], dtype=np.int32)
    raw = tifffile.imread(str(raw_files[min(idx, len(raw_files)-1)])) if raw_files else np.zeros(ov.shape[:2], dtype=np.uint8)
    p1, p99 = np.percentile(raw[raw > 0], [1, 99]) if raw.max() > 0 else (0, 1)
    raw_norm = np.clip((raw.astype(np.float32) - p1) / (p99 - p1 + 1e-6) * 255, 0, 255).astype(np.uint8)
    ...
    def rh(arr, h):
        img = Im.fromarray(arr); sc = h / img.height
        return np.array(img.resize((int(img.width * sc), h), Im.LANCZOS))

    raw_r = rh(raw_crop, H); vib_r = rh(vib_crop, H)
    ...
    draw.text((raw_r.shape[1] + 14, 14), 'Brainfast — Atlas Registration (Allen CCFv3)', fill=(200, 200, 200), font=font)
...
def main():
    ...
    ap.add_argument('--slice', type=int, default=None, help='Slice index for best-slice image (default: auto-pick)')
    ...
    if total == 0:
        print('ERROR: No registered slices found. Run the pipeline first.'); sys.exit(1)
    ...
    p = make_panel(
        ...
        n_slices  = args.panel_n, cols=4, thumb_size=380,
    ...
    if best_lbl.exists() and structure_csv.exists():
        ...
        print(f'  demo_annotated_slice.jpg  (slice {best_idx}, {len(np.unique(tifffile.imread(str(best_lbl))))-1} regions)')
    ...
    if hier_path.exists():
        ...
        if not root.empty:
            ...
            for _, r in d2.iterrows():
                ...
                print(f'    {r["acronym"]:<8} {r["region_name"][:30]:<30}  {int(r["count"]):>8,}  ({pct:.1f}%)')
