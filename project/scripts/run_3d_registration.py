...
def stack_tifs_to_nifti(slice_dir, output_path, pixel_um_xy=5.0,
                         z_spacing_um=25.0, target_um=25.0):
...
def prepare_cropped_template(full_path, annot_path, hemisphere,
                              ap_start, ap_end, out_dir):
    ...
    for src, dst, is_label in [(full_path, tmpl_out, False),
                                 (annot_path, ann_out, True)]:
        if dst.exists():
...
def run_transformix(annotation, transform_params, out_dir):
    ...
    tp = tp.replace('(FinalBSplineInterpolationOrder 3)',
                    '(FinalBSplineInterpolationOrder 0)')
    tp = tp.replace('(ResampleInterpolator "FinalBSplineInterpolator")',
                    '(ResampleInterpolator "FinalNearestNeighborInterpolator")')
    patched = out_dir / "TransformParameters_NN.txt"
    patched.write_text(tp, encoding="utf-8")
    cmd = [str(TRANSFORMIX_EXE),
           "-in", str(annotation),
           "-out", str(out_dir),
           "-tp", str(patched),
           "-threads", "8"]
...
def main():
    ...
    out_dir      = Path(args.out_dir) if args.out_dir else \
                   PROJECT_DIR / "outputs" / "registration_3d"
    ...
    if not brain_nii.exists():
        ...
        _, brain_shape = stack_tifs_to_nifti(
            slice_dir, brain_nii, pixel_um_xy, z_spacing)
    else:
    ...
    tmpl_cropped, ann_cropped = prepare_cropped_template(
        TEMPLATE_25, ANNOTATION, hemisphere, ap_start, ap_end,
        out_dir)
    ...
    print(f"Run test_single_slice.py to view overlay.")
    print(f"{'='*60}")
