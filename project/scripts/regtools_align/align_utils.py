...
Vendored into Brainfast on 2026-04-16 as project/scripts/regtools_align/align_utils.py.
Minimally modified from upstream:
  - Dropped `from regtools.utils.io.nifti_utils import load_nii_pir` and
    `load = load_nii_pir` — Brainfast passes numpy arrays directly, never
    disk paths, so the loader is unused here.
  - Inlined `preprocess` (17-line helper from
    regtools.utils.image_processing) so the vendored package is
    self-contained; behaviour identical.
Do not otherwise edit locally; update by re-syncing from upstream.

--- Original header ---
Axis alignment utilities for brain volume orientation.

Provides functions for detecting the longitudinal fissure,
...
# Inlined from regtools.utils.image_processing.preprocess — see vendor notes above.
def preprocess(img: np.ndarray, min_val: float = None, max_val: float = 400) -> np.ndarray:
    """Clip *img* above *max_val* (and optionally below *min_val*), then
    normalise to uint8 0–255 for Otsu / Canny / Hough detection."""
    data = img.copy()
...
def show(img, title=""):
    """Display an image with matplotlib (used by debug flags)."""
    plt.figure()
...
def svd_fit(filepath="output/fissure_pts.npy", debug=True):
    ...
    z_grid = (-a * x_grid - b * y_grid + d) / c if c != 0 else np.zeros_like(x_grid)
