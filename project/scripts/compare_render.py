...
def render_before_after(real_path: Path, before_label_path: Path, after_label_path: Path, out_path: Path, alpha: float = 0.45, before_score: float | None = None, after_score: float | None = None) -> Path:
    ...
    if b.shape != real.shape:
        b = resize(b.astype(np.float32), real.shape, order=0, preserve_range=True, anti_aliasing=False).astype(np.uint16)
    if a.shape != real.shape:
        a = resize(a.astype(np.float32), real.shape, order=0, preserve_range=True, anti_aliasing=False).astype(np.uint16)
    ...
    if before_score is not None and after_score is not None:
        ...
        draw.text((10, h - 22), f"SSIM before={before_score:.4f}  after={after_score:.4f}", fill=(255, 255, 255))
