...
def outputs_demo_best_slice():
    ...
    if run_dir is not None:
        try:
            ...
            return jsonify({"ok": False, "error": f"3D best-slice generation failed: {exc}", "error_code": ERR_INTERNAL}), 500

    outputs_root = _outputs_root()
    fp = outputs_root / "demo_best_slice.jpg"
    if not fp.exists():
        return jsonify({"ok": False, "error": "Best-slice image not generated yet", "error_code": ERR_NOT_FOUND}), 404
...
def outputs_cell_chart():
    ...
    if not hier_path.exists():
        return jsonify({"ok": False, "error": "No hierarchy CSV yet", "error_code": ERR_NOT_FOUND}), 404
    sources = [hier_path]
    ...
    if not chart_path.exists() or any(
        ...
        try:
            ...
            return jsonify({"ok": False, "error": f"Chart generation failed: {exc}", "error_code": ERR_INTERNAL}), 500
...
def outputs_cell_summary():
    ...
    try:
        ...
        return jsonify({"ok": False, "error": f"Cell summary generation failed: {exc}", "error_code": ERR_INTERNAL}), 500
...
def outputs_demo_comparison(slice_idx: int):
    ...
    if not ov_path.exists():
        return jsonify({"ok": False, "error": f"slice {slice_idx} not found", "error_code": ERR_NOT_FOUND}), 404
...
def outputs_demo_panel():
    ...
    if run_dir is not None:
        try:
            ...
            return jsonify({"ok": False, "error": f"3D panel generation failed: {exc}", "error_code": ERR_INTERNAL}), 500
    ...
    if not panel_path.exists() or (
        ...
        try:
            ...
            return jsonify({"ok": False, "error": f"Panel generation failed: {exc}", "error_code": ERR_INTERNAL}), 500
...
def outputs_refresh_demo():
    ...
    if run_dir is None and not script.exists():
        return jsonify({"ok": False, "error": "refresh_demo.py not found", "error_code": ERR_NOT_FOUND}), 404
...
def outputs_reg_stats():
    ...
    if not qc_path.exists():
        return jsonify({"ok": False, "error": "No registration QC data yet", "error_code": ERR_INTERNAL})
    try:
