...
Blueprints call these functions; heavy imports (matplotlib, PIL, tifffile) live
here, not in route handlers. All functions take plain Path arguments and raise
on failure.
"""
...
def generate_cell_chart(hier_path: Path, chart_path: Path, project_root: Path) -> None:
    """Generate a cell-count bar+pie chart."""
...
def generate_demo_comparison(
    ...
    """Generate a side-by-side raw-vs-atlas comparison image."""
    import numpy as np
    ...
    with Image.open(str(ov_path)) as handle:
        ov = np.array(handle.convert("RGB"))
    ...
    def _resize_h(arr: np.ndarray, target_height: int) -> np.ndarray:
        img = Image.fromarray(arr)
        scale = target_height / img.height
        return np.array(img.resize((int(img.width * scale), target_height), Image.LANCZOS))
    ...
    draw.text(
        (raw_r.shape[1] + 14, 14), "Brainfast - Atlas Registration", fill=(200, 200, 200), font=font
    )
    draw.text(
        (width - 260, 32), f"Slice {slice_idx} | Allen CCFv3", fill=(100, 100, 100), font=small
    )
